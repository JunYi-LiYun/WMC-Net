...
def cross_entropy_loss_one_hot(logits, target, reduction='mean'):
    logp = F.log_softmax(logits, dim=1)   # 归一化logits为0-1的数字 而且都代表他们的占比 概率分配 log就是在上面做了一次log运算，dim=1：在第1维上sum=1
...
def one_hot_encoding(label, n_classes):
    # scatter_(dim, index, src) → Tensor：可以理解成放置元素或者修改元素，
    # dim：沿着哪个维度进行索引 index：用来 scatter 的元素索引  src：用来 scatter 的源元素，可以是一个标量或一个张量
    return torch.zeros(label.size(0), n_classes).to(label.device).scatter_(0, label.view(-1, 1), 1)  # 返回一个形状为为label.size(0),里面的每一个值都是0的tensor
#
# def label_smoothing_criterion(alpha=0.1, distribution='uniform', std=0.5, reduction='mean'):
def label_smoothing_criterion(logits, labels,alpha=0.1, distribution='uniform', std=0.5, reduction='mean'):
        ...
        if distribution == 'uniform':  # 均匀分布
            uniform = torch.ones_like(one_hot).to(device)/n_classes    # 根据给定张量one_hot，生成与其形状相同的全1张量
